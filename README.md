...
- Environment: adicione as variáveis do arquivo `server/.env.example` no painel do Render
...
- Build Command: vazio
- Output Directory: `.`
- Variável opcional: nenhuma obrigatória, porque a URL da API pode ser preenchida no próprio campo da interface
...
No GitHub, deixe `client` e `server` na raiz do repositório. Não deixe uma pasta extra `onepiece-ai/` envolvendo tudo, porque isso só complica o deploy no Vercel e no Render.
